defmodule Jehuty.Admin.AdminUserToken do
  ...
  schema "admin_user_tokens" do
    field :token, :binary
    field :context, :string
    field :sent_to, :string
    belongs_to :admin_user, Jehuty.Admin.AdminUser
  ...
  @doc """
  Builds a token and its hash to be delivered to the admin_user's email.

  The non-hashed token is sent to the admin_user email while the
      ...
      {:ok, decoded_token} ->
        ...
        query =
          from token in token_and_context_query(hashed_token, context),
            join: admin_user in assoc(token, :admin_user),
            where: token.inserted_at > ago(^days, "day") and token.sent_to == admin_user.email,
            select: admin_user
  ...
  This is used to validate requests to change the admin_user
  ...
  def token_and_context_query(token, context) do
    from AdminUserToken, where: [token: ^token, context: ^context]
  end
  ...
  def admin_user_and_contexts_query(admin_user, :all) do
    from t in AdminUserToken, where: t.admin_user_id == ^admin_user.id
  end

  def admin_user_and_contexts_query(admin_user, [_ | _] = contexts) do
    from t in AdminUserToken, where: t.admin_user_id == ^admin_user.id and t.context in ^contexts
  end
end
