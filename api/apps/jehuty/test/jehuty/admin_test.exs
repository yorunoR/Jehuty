defmodule Jehuty.AdminTest do
  use Jehuty.DataCase

  alias Jehuty.Admin

  import Jehuty.AdminFixtures
  alias Jehuty.Admin.{AdminUser, AdminUserToken}
  ...
  describe "register_admin_user/1" do
    ...
    test "registers admin_user with a hashed password" do
  ...
  describe "apply_admin_user_email/3" do
    ...
    test "validates email uniqueness", %{admin_user: admin_user} do
      ...
      {:error, changeset} = Admin.apply_admin_user_email(admin_user, password, %{email: email})
    ...
    test "sends token through notification", %{admin_user: admin_user} do
      token =
        extract_admin_user_token(fn url ->
          Admin.deliver_admin_user_update_email_instructions(admin_user, "current@example.com", url)
        end)
  ...
  describe "delete_admin_user_session_token/1" do
    test "deletes the token" do
      admin_user = admin_user_fixture()
      token = Admin.generate_admin_user_session_token(admin_user)
      assert Admin.delete_admin_user_session_token(token) == :ok
      refute Admin.get_admin_user_by_session_token(token)
    end
  end
